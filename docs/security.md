...
Protect the account running `bashido`, its configuration directory, editor configuration, custom CA files, and server. Use `auth logout` to revoke one credential or `uninstall --yes` to revoke every saved credential and remove the CLI. Use `--local-only` only when a server is unreachable; the CLI warns that the server credential may remain active, and you must revoke it separately later.
